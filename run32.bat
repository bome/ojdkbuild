...
rem patch nspr and nss packages for the build to complete
copy resources\patches\lookaside\nspr\ntinrval.c lookaside\nspr\pr\src\md\windows\ntinrval.c
copy resources\patches\lookaside\nss\mpi.c lookaside\nss\lib\freebl\mpi\mpi.c

call "%OJDKBUILD_DIR%/resources/scripts/set-compile-env%BUILD_BITS%.bat"
@echo off
...
rem $$fb nmake zip || exit /b 1
rem $$fb nmake installer || exit /b 1
